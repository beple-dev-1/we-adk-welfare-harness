...
모든 `@RequestBody` / `@ModelAttribute` DTO에 `@Valid` 어노테이션 적용 의무.
검증 실패 시 `MethodArgumentNotValidException`이 자동으로 발생하고, `GlobalExceptionHandler`가 `ApiResponse` 400 오류 응답으로 변환한다.
**`BindingResult`를 Controller 파라미터로 직접 받는 방식은 이 프로젝트에서 사용하지 않는다.**

```java
@PostMapping("/api/v1/ceremony/apply")
public ResponseEntity<ApiResponse<CeremonyApplyResponse>> apply(
        @Valid @RequestBody CeremonyApplyRequest req) {
    // @Valid 검증 실패 → MethodArgumentNotValidException 자동 발생
    // GlobalExceptionHandler 가 ApiResponse 400 응답으로 자동 변환
    // BindingResult 파라미터 불필요 — 직접 처리하지 않는다
    return ResponseEntity.ok(ApiResponse.success(service.apply(req)));
}
```
...
| GlobalExceptionHandler 위임 | `BindingResult`를 Controller에서 직접 처리하지 않고 `GlobalExceptionHandler`에 위임하는가 |
